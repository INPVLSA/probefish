...
export function ValidationRulesEditor({
  ...
  // Dialog for adding/editing rules (shared between compact and full modes)
  const ruleDialog = (
  ...
  // Rules list content (shared between compact and full modes)
  const rulesListContent = rules.length === 0 ? (
    <div className={`text-center text-muted-foreground text-sm ${compact ? "py-3" : "py-6"}`}>
      {compact ? "No rules defined" : "No validation rules. Output will always pass."}
    </div>
  ) : (
    <div className={compact ? "space-y-1.5" : "space-y-2"}>
      {rules.map((rule, index) => {
        ...
        return (
          <div
            key={index}
            className={`flex items-center gap-2 ${compact ? "p-2" : "p-3"} rounded-lg group ${
              isWarning
                ? "bg-amber-500/10 border border-amber-500/20"
                : "bg-destructive/10 border border-destructive/20"
            }`}
          >
            <div className={`${compact ? "h-6 w-6" : "h-8 w-8"} rounded-md flex items-center justify-center ${
              isWarning ? "bg-amber-500/20" : "bg-destructive/20"
            }`}>
              {isWarning ? (
                <AlertTriangle className={compact ? "h-3 w-3 text-amber-500" : "h-4 w-4 text-amber-500"} />
              ) : (
                <ShieldCheck className={compact ? "h-3 w-3 text-destructive" : "h-4 w-4 text-destructive"} />
              )}
            </div>
            <div className="flex-1 min-w-0">
              ...
              <div className={`text-muted-foreground truncate ${compact ? "text-xs" : "text-xs"}`}>
